...
<head>
    ...
    background-color: rgb(255, 255, 255);
      ...
      .top_order3 > .top_order_prod3 > img{
    position: relative;
    top: 0px;
    height: 172px;
    width: 200px;
...
<body>
  <div class="top_order3">
  <div class="top_order3_name">Beverages</div>
     <div class="top_order_prod3">
     <?php  
         $var = 1;
             
            $dbconnect = mysqli_connect("localhost","root","","foodpoint");
            //preparing sql query to retrieve data from database
            $sql = "SELECT * FROM beverages where id=$var";
            $r = mysqli_query($dbconnect,$sql);
            while($rows = mysqli_fetch_assoc($r)){
               echo '<img src="/FoodPoint/FoodPoint_IMG/'.$rows["image_lct"].'" alt="">';
                 
              echo '<div class="top_order_label">';
                echo '<span class="prod3name">'.$rows["drink_name"].'</span>';
                echo '<span class="prod3price"><span>&#8358;</span>'.$rows["drink_price"].'</span>';
            ...
            echo '<form action="/FoodPoint/FoodPoint_Cart/cart.php" method="get">
              ...
              <input type="hidden" value="'.$rows["drink_name"].'" name="nam">
              <input type="hidden" value="'.$rows["drink_price"].'">
